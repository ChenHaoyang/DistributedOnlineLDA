...
class DistributedOnlineLDA(params: OnlineLDAParams) extends OnlineLDA with Serializable {
  ...
  override def eStep(
    ...
    val eStepResult = removedDocId
      .map(wIdCtRow => {
        ...
        val currentTopicsMatrix = new BDM( //T * V
          params.numTopics,
          ...
          true
                                                           ...
                                                           //T * V
        val ELogBetaDoc = Utils.dirichletExpectation(currentTopicsMatrix, lambdaSumbd.value).t   //V * T
        oneDocEStep(
  ...
  private def initLambda(implicit sc:SparkContext): IndexedRowMatrix = {
    val topics = sc.parallelize(Array.range(0, params.vocabSize-1), 8)
